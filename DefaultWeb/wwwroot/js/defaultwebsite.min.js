 ...
 * Based on Bootstrap
*/
define("dws/model",["dws/model-actions"],function(n){var t={file:ko.observable(""),files:ko.observableArray([]),data:ko.observable(""),dataType:ko.observable(""),target:ko.observable(""),abort:function(t,i,r){n.callAborted(t,i,r)},waiting:ko.observable(!1),waitingTarget:ko.observable(""),sources:ko.observableArray([]).extend({deffer:!0}),source:ko.observable(""),sourceId:function(n){var i=t.sources().findIndex(t=>t.id==n);return t.sourceIndex(i),t.source(t.sources()[i]),t.source()},sourceIndex:ko.observable(""),sourcesCount:ko.pureComputed(function(){return"Records: "+t.sources().length},this),addSource:function(n){t.sources.unshift(n);t.sourceId(n.id)},canDeleteSource:function(){return t.sources().length>0},removeSource:function(n){var r=t.sources().findIndex(t=>t.id==n),i=t.sources()[t.sourceIndex()];t.sources.remove(i);t.source(undefined)},sourceAdded:function(n,t,i){var u=$(n),r=$(i);r.hide().fadeIn("slow");console.log("Source afterAdd... ")},comments:ko.observableArray([]).extend({deffer:!0}),comment:ko.observable(""),commentId:function(n){var i=t.comments().findIndex(t=>t.id==n);return t.commentIndex(i),t.comment(t.comments()[i]),t.comment()},commentIndex:ko.observable(""),commentsCount:ko.pureComputed(function(){return"Records: "+t.comments().length},this),commentTitle:ko.pureComputed(function(){return"Ima a stupid title!"},this),addComment:function(n){t.comments.unshift(n);t.commentId(n.id)},removeComment:function(n){var r=t.comments().findIndex(t=>t.id==n),i=t.comments()[t.commentIndex()];t.comments.remove(i);t.comment(undefined)},commentAdded:function(n){var t=$(n);t.hide().fadeIn("slow");console.log("Comment afterAdd... ")},canAddComment:function(){return t.sources().length===0?!1:!0}};return t.data.subscribe(function(n){t.target()?$(t.target()).html(n):$(n).dialog()}),t.source.subscribe(function(n){if(n==undefined){var i=t.sourceIndex()-1;i>=0&&(n=t.sources()[i],t.sourceId(n.id));return}$($("#sources-table tbody tr")[t.sourceIndex()]).addClass("active").siblings().removeClass("active");t.comments(n.comments==null?[]:n.comments);t.comments().length>0&&t.commentId(t.comments()[0].id);console.log("source subscribe:"+n.sourceName)}),t.comment.subscribe(function(n){var i,u,r;n==undefined?(i=t.commentIndex()-1,i>=0&&(n=t.comments()[i],t.commentId(n.id))):(u=$("#comments.list-group").children(),r=$(u[t.commentIndex()]),r.addClass("active").siblings().removeClass("active"),$(".comment.card a#comment-delete").addClass("disabled"),r.find("a#comment-delete").removeClass("disabled"),console.log("comment subscribe:"+n.id))}),t.waiting.subscribe(function(i){n.waitStatus(i,t.waitingTarget())}),t});define("dws/model-actions",function(){function n(n,t){n?$(t).addClass("waiting"):$(t).removeClass("waiting")}function t(n,t){var i=$(t)[0].complete;i||($(t)[0].addClass("loading"),$(t).load(function(){$(t).removeClass("loading")}))}return{waitStatus:n,loadingStatus:t}});define("dws/dispatcher",["dws/model"],function(n){function t(n){var t=new XMLHttpRequest;t.onload=function(){this.readyState==4&&this.status==200?alert(this.responseText):alert("Message not found!"+this.readyState+":"+this.status+"-"+this.responseText)};t.open("GET",n,!0);t.send()}function i(t){$.ajax(t).done(function(t){n.data(t)}).fail(function(t,i){n.aborted(t,i,this.responseText)}).always(function(){})}function r(n,t){$.ajax(n).done(function(n){t.resolve(n)}).fail(function(n,i){t.reject(this.responseText+"\n"+i)}).always(function(){})}return{xhrRequest:t,ajaxRequest:i,ajaxRequestDefer:r}});define("dws/actions",["dws/controller"],function(n){return $(document).ready(function(){$(".rundown").on("click",function(t){t.preventDefault();var i=$(this);$(".rundown").removeClass("selected");i.addClass("selected");n.sendMessage(i)});$(".nav-link").on("click",function(t){t.preventDefault();var i=$(this);n.sendMessageDefer(i)});$(document).on("click",".sand-link",function(t){t.preventDefault();var i=$(t.target);$(".sand-link").removeClass("active");i.addClass("active");n.sendMessageDefer(i)});$(document).on("click","#btn-blog",function(n){n.preventDefault();$("#blog-text").toggleClass("hidden");$("#blog-content").toggleClass("hidden")});$(document).on("shown.bs.collapse","#doc-resume",function(n){n.preventDefault();$("#contact.card").animate({scrollTop:$(this).offset().top},800);$('[data-target="#doc-resume"] h4 i').switchClass("fa-eye","fa-eye-slash");$("#doc-cv").hasClass("show")&&$("#doc-cv").removeClass("show")});$(document).on("hide.bs.collapse","#doc-resume",function(){$('[data-target="#doc-resume"] h4 i').switchClass("fa-eye-slash","fa-eye")});$(document).on("hide.shown.bs.collapse","#doc-cv",function(n){n.preventDefault();$("#contact.card").animate({scrollTop:$(this).offset().top},800);$('[data-target="#doc-cv"] button h4 i').switchClass("fa-eye","fa-eye-slash");$("#doc-resume").hasClass("show")&&$("#doc-resume").removeClass("show")});$(document).on("bs.collapse","#doc-cv",function(n){n.preventDefault();$('[data-target="#doc-cv"] h4 i').switchClass("fa-eye-slash","fa-eye")});$(document).on("show.bs.modal","#modal-action-template",function(t){var i=$(t.relatedTarget),r=$(this);r.find(".modal-title").text("Add New "+i.attr("data-target-id"));n.sendMessage(i,"#target-modal")});$(document).on("shown.bs.modal","#modal-action-template",function(n){var t=$(n.target);t.find("input:visible").first().focus()});$("#rundown-info").popover({trigger:"click",title:"What is a Run-down???",content:"A Run-down is a casual non-authoritative white-paper. In my words and IMHO...",placement:"bottom",delay:{show:200,hide:100}});$("div#footer-scroll").endlessScroll({width:"100%",height:"20px",steps:-2,speed:40,mousestop:!0})}),{}});define("dws/sandbox",["dws/model"],function(n){function r(n){n?t.observe(document.getElementById("sandbox-area"),i):t.disconnect()}var i={attributes:!0,childList:!0,characterData:!0},t=new MutationObserver(function(t){t.forEach(function(t){if(t.addedNodes.length>0){var i=$(t.addedNodes).find("[data-bind]");i.each(function(){var t=$(this);try{ko.dataFor(t[0])||ko.applyBindings(n,t[0])}catch(i){console.log("ko re-bind exception....")}})}})});return{observe:r}});define("dws/controller",["dws/model","dws/dispatcher"],function(n,t){function u(){n.target(null);t.ajaxRequest({url:"/Home/Talk?message=Hello Server",cache:!1,dataType:"html"})}function f(n){$(n).is(":visible")||(r(),$(n).show())}function r(){$(".content-area").hide()}function i(n){var t;return t=n.attr("data-target-controller")&&n.attr("data-target-action")?"/"+n.attr("data-target-controller")+"/"+n.attr("data-target-action"):"/Home/GetView",n.attr("data-target-id")&&(t=t+"?id="+n.attr("data-target-id")),t}function e(r,u){u||(u=r.attr("data-target"));var f=i(r),e={url:f,cache:!1};n.target(u);t.ajaxRequest(e)}function o(r){var f=r.attr("data-target"),e=i(r),o={url:e,cache:!1},u=new $.Deferred;u.done(function(t){t?n.data(t):alert("invalid data from server")}).fail(function(n){alert(n)});n.target(f);t.ajaxRequestDefer(o,u)}function s(){ko.applyBindings(n)}return{initKO:s,test:u,sendMessage:e,sendMessageDefer:o,showContentArea:f,hideAllContent:r,parseNavUrl:i}});define("dws/comments",["dws/controller","dws/model"],function(n,t){function r(){$.ajax({url:"/Comments/GetSources",cache:!1,dataType:"json"}).done(function(n){t.sources([]);t.sources(n);n.length>0&&t.sourceId(n[0].id)}).fail(function(n,i){t.aborted(n,i,this.responseText)}).always(function(){})}function i(){$.ajax({url:"/Comments/DeleteSource",cache:!1,data:t.sourceId()}).done(function(n,i,r){r.status==200&&t.removeSource(n)}).fail(function(n,i,r){t.abort(n,i,r)})}function u(){$.ajax({url:"/Comments/DeleteComment?id="+t.comment().id,cache:!1,data:t.commentId()}).done(function(n,i,r){r.status==200&&t.removeComment(n)}).fail(function(n,i,r){t.abort(n,i,r)})}$(document).on("click","#sources-table tbody tr",function(n){var i=$(n.currentTarget),r=i.attr("id");t.sourceId(r)});$(document).on("click",".comment.card",function(n){n.preventDefault();var i=$(n.currentTarget),r=i.attr("id");t.commentId(r)});$("#modal-action-template").on("show.bs.modal",function(t){var i=$(t.relatedTarget);n.sendMessage(i,"#modal-action-template")});$(document).on("submit","form#create-source",function(n){var i,r;n.preventDefault();i=$(this);i.attr("disabled",!0);r=i.serializeArray();$.ajax({url:"/Comments/CreateSource",type:"POST",data:r}).done(function(n,r,u){u.status==200?(i.closest("#modal-action-template").modal("hide"),t.addSource(n.source)):$("#target-modal").html(n)}).fail(function(){alert("error")})});$(document).on("submit","form#create-comment",function(n){var i,r;n.preventDefault();i=$(this);i.attr("disabled",!0);$("input#SourceId.form-control",i).val(t.source().id);r=i.serializeArray();t.waitingTarget(".modal-header");t.waiting(!0);$.ajax({url:"/Comments/CreateComment",type:"POST",data:r}).done(function(n,r,u){u.status==200?(i.closest("#modal-action-template").modal("hide"),t.addComment(n.comment)):$("#target-modal").html(n)}).fail(function(n,r,u){i.closest("#modal-action-template").modal("hide");t.abort(n,r,u)}).always(function(){t.waiting(!1)})});$(document).on("click","a#source-delete",function(n){n.preventDefault();t.comments().length>0?$.confirm({title:"Cascade Delete Source and Comments?",content:"There are child Comments! Continuing will delete the Source record and all child Comments. This action can not be undone!!!",buttons:{confirm:function(){i()},cancel:function(){return}}}):i()});$(document).on("click","a#comment-delete",function(n){n.preventDefault();$.confirm({title:"Delete Comment(s)?",content:"This action can not be undone!!!",buttons:{confirm:function(){u()},cancel:function(){return}}})});return{GetSources:r}});require(["dws/actions"]);require(["dws/comments"]);require(["dws/sandbox"]);require(["dws/model-actions"]);require(["dws/controller"]),function(n){$(document).ready(function(){n.initKO()})};