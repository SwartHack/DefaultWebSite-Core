 ...
 * Based on Bootstrap
*/
define("dws/komodel",function(){var n={file:ko.observable(""),files:ko.observableArray(),data:ko.observable(""),dataType:ko.observable(""),target:ko.observable(""),error:ko.observable(function(n,t,i){alert(i+"\n"+t)}),waitEffects:ko.observable(!1)};return n.data.subscribe(function(t){n.target()?$(n.target()).html(t):$(t).dialog()}),n});define("dws/dispatcher",["dws/komodel"],function(n){function t(n){var t=new XMLHttpRequest;t.onload=function(){this.readyState==4&&this.status==200?alert(this.responseText):alert("Message not found!"+this.readyState+":"+this.status+"-"+this.responseText)};t.open("GET",n,!0);t.send()}function i(t){$.ajax(t).done(function(t){n.data(t)}).fail(function(t,i){n.aborted(t,i,this.responseText)}).always(function(){})}function r(n,t){$.ajax(n).done(function(n){t.resolve(n)}).fail(function(n,i){t.reject(this.responseText+"\n"+i)}).always(function(){})}return{xhrRequest:t,ajaxRequest:i,ajaxRequestDefer:r}});define("dws/actions",["dws/controller"],function(n){function i(n){$(n).is(":visible")||(t(),$(n).show())}function t(){$(".content-area").hide()}return $(document).ready(function(){$(document).on("shown.bs.collapse","#doc-resume",function(n){n.preventDefault();$("#target-area").animate({scrollTop:$(this).offset().top},800);$('[data-target="#doc-resume"] h4 i').switchClass("fa-eye","fa-eye-slash");$("#doc-cv").hasClass("show")&&$("#doc-cv").removeClass("show")});$(document).on("hide.bs.collapse","#doc-resume",function(){$('[data-target="#doc-resume"] h4 i').switchClass("fa-eye-slash","fa-eye")});$(document).on("hide.shown.bs.collapse","#doc-cv",function(n){n.preventDefault();$("#target-area").animate({scrollTop:$(this).offset().top},800);$('[data-target="#doc-cv"] button h4 i').switchClass("fa-eye","fa-eye-slash");$("#doc-resume").hasClass("show")&&$("#doc-resume").removeClass("show")});$(document).on("bs.collapse","#doc-cv",function(){$('[data-target="#doc-cv"] h4 i').switchClass("fa-eye-slash","fa-eye")});$("#rundown").on("click",function(t){var i,r;t.preventDefault();i=$(this);$("li.rundown").removeClass("selected");i.addClass("selected");r={url:"/Home/GetRundown?viewname="+i.attr("data-target-view"),cache:!1,dataType:"html"};n.sendMessage(r,"#target-area")});$(".nav-link").on("click",function(t){t.preventDefault();var i={url:"/Home/GetView?viewname="+$(this).attr("data-target-view"),cache:!1,dataType:"html"};n.sendMessageDefer(i,"#target-area")});$(document).on("click","#btn-blog",function(n){n.preventDefault();$("#blog-text").toggleClass("hidden");$("#blog-content").toggleClass("hidden")});$("input:checkbox").change(function(){});$("div#footer-scroll").endlessScroll({width:"100%",height:"20px",steps:-2,speed:40,mousestop:!0})}),{showContentArea:i,hideAllContent:t}});define("dws/sandbox",function(){});define("dws/controller",["dws/komodel","dws/dispatcher"],function(n,t){function i(){n.target(null);t.ajaxRequest({url:"/Home/Talk?message=Hello Server",cache:!1,dataType:"html"})}function r(i,r){n.target(r);t.ajaxRequest(i)}function u(i,r){var u=new $.Deferred;u.done(function(t){t?n.data(t):alert("invalid data from server")}).fail(function(n){alert(n)});n.target(r);t.ajaxRequestDefer(i,u)}function f(){ko.applyBindings(n)}return{initKO:f,test:i,sendMessage:r,sendMessageDefer:u}});require(["dws/actions"]);require(["dws/controller"],function(n){$(document).ready(function(){n.initKO()})});